(*
 * Added a getDateTime function to print the current datetime, and append that to each of the
 * messages. No additional libraries required other than the command provided on the hw5.md:
     * ocamlbuild -lib str -pkgs lwt,lwt.unix funSrv.native
 *)
...
module Server = struct
    ...
    let getDateTime = let t = Unix.localtime (Unix.time ()) in
        "    <" ^
        string_of_int (1900 + t.tm_year) ^ "-" ^
        string_of_int (1 + t.tm_mon) ^ "-" ^
    ...
    let remove_session nn = 
      ...
      broadcast nn ("<left chat>" ^ getDateTime) >>= fun () ->
      Lwt.return ()
    ...
    let change_nn nn outp new_nn = (
        ...
        broadcast new_nn ("<" ^ nn ^ " change nick to " ^ new_nn ^ ">" ^ getDateTime))
    ...
    let chat_handler (inp,outp) =
      ...
      let initiate = Lwt_io.write_line outp "Enter initial nick:" >>=
          fun () -> Lwt_io.read_line inp >>= 
              fun l -> (nick := l ; broadcast l ("<joined>" ^ getDateTime) ;
      ...
      | _ -> broadcast !nick (l ^ getDateTime) in
